 ...
 * 
 * Requirements:
 * 	-Satisfies all part 2 Requirements
 *
 * Run:
 * 	This is the main entry point to the program right click this class in the explorer then run as java application
 */

//This the GUI class that populates data from the controller and acts as
//an interface between the user and the controller.
public class GuiController {

	public static void main(String[] args) {
		GuiController greenhouseGui = new GuiController(true);
	}
	...
	public GuiController(boolean mainWindow) {
	...
	private void setCurrentController(GreenhouseControls greenhouseControls) {
		this.currentController = greenhouseControls;
	}
	...
	private void initializeGUI() {
		...
		// Setup main window
		windowFrame.setTitle("Athabasca University Greenhouse Controller");
		windowFrame.setIconImage(greenhouseIcon.getImage());
		windowFrame.setPreferredSize(new Dimension(800, 600));
		// mainWindowFrame.setLocationRelativeTo(null);
		windowFrame.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
		windowFrame.setResizable(false);

		createLayoutAndDisplay(eventTextAreaScrollPane, startButton, restartButton, terminateButton, suspendButton,
				resumeButton, currentFileField);

	}

	// Instantiate icons, formatted for 20x20 pixel sizes
	private void createIcons() {
		greenhouseIcon = new ImageIcon("src/icons/au.jpg");
		exitIcon = new ImageIcon("src/icons/error.png");
		newIcon = new ImageIcon("src/icons/new.png");
		openIcon = new ImageIcon("src/icons/folder.png");
		saveIcon = new ImageIcon("src/icons/logout.png");
		restartIcon = new ImageIcon("src/icons/restart.png");
		terminateIcon = new ImageIcon("src/icons/error.png");
		startIcon = new ImageIcon("src/icons/power.png");
		suspendIcon = new ImageIcon("src/icons/suspend.png");
		resumeIcon = new ImageIcon("src/icons/resume.png");
	}
	...
	private void createMenuBar() {
		...
		exitMenuItem.addActionListener((ActionEvent event) -> {
			if (currentController.isRunning()) {
				int response = JOptionPane.showConfirmDialog(null, "Exit Program?", "Confirm Exit",
		...
		saveMenuItem.setToolTipText("Close controller");
		saveMenuItem.addActionListener((ActionEvent event) -> {

			if (currentController.isRunning()) {
				int response = JOptionPane.showConfirmDialog(null,
						"Are you sure you want to close the current controller?", "Confirm Close",
		...
		openMenuItem.addActionListener((ActionEvent event) -> {
			openCheckAndReadEventFile(eventFileChooser.showOpenDialog(windowFrame));
		});
		...
		restartMenuItem.addActionListener((ActionEvent event) -> {
			openCheckAndReadDumpFile(dumpFileChooser.showOpenDialog(windowFrame));
		...
		windowFrame.setJMenuBar(topFileMenu);
	}
	...
	private void openCheckAndReadEventFile(int result) {
		if (result == JFileChooser.APPROVE_OPTION) {
			...
			if (!extension.equals("txt")) {
				currentFileField.setText("Please select a Event (.txt) file");
				return;
			}
			// Set the gui element to display the current file
			...
			try {
				...
				while (populateActionsScanner.hasNextLine()) {
					...
					if (words.length == 2) {
						...
						currentController.addRepeatingEvent(words[0], new Long(words[1]), new Integer(words[2]));
					} else {
	...
	private void createGuiElements() {
		...
		currentFileField.setOpaque(false);
		...
		eventFileChooser.setCurrentDirectory(new File(System.getProperty("user.dir")));
		eventFileChooser.setFileFilter(new FileNameExtensionFilter("Event Files (.txt)", "txt"));

		dumpFileChooser = new JFileChooser();
		dumpFileChooser.setCurrentDirectory(new File(System.getProperty("user.dir")));
		dumpFileChooser.setFileFilter(new FileNameExtensionFilter("Dump Files (.out)", "out"));
	...
	private void createPopupMenu() {
		...
		windowFrame.addMouseListener(new PopupMenuMouseListener());
	}
	...
	class PopupMenuMouseListener extends MouseAdapter {
		...
		private void checkPopup(MouseEvent e) {
			if (e.isPopupTrigger()) {
				rightClickPopupMenu.show(windowFrame, e.getX(), e.getY());
			}
		}
	}
	...
	private void createLayoutAndDisplay(JComponent... args) {
		...
		addComponent(args[1], 0, 0);
		addComponent(args[2], 0, 1);
		addComponent(args[3], 0, 2);
		addComponent(args[4], 0, 3);
		addComponent(args[5], 0, 5);

		// File information and setting
		// the border.
		// gridBagConstraints.weightx = 1.0;
		// gridBagConstraints.weighty = 1.0;
		gridBagConstraints.insets = new Insets(10, -128, 10, 10);
		addComponent(args[6], 5, 11);
		currentFileFieldBorder = BorderFactory.createTitledBorder(null, "Events File", TitledBorder.CENTER,
				TitledBorder.TOP, new Font("Dialog", Font.BOLD, 16), Color.BLACK);
	...
	private void addComponent(Component component, int x, int y) {
		...
		windowFrame.add(component, gridBagConstraints);
	}
	...
	public void outputGreenhouseStates() {
		Iterator<GreenhouseControls.ControllerState> stateIterator = getControllerState().iterator();
		while (stateIterator.hasNext()) {
			eventTextArea.append(stateIterator.next().toString() + "\n\r");
		}
	}
	...
	private void readGreenhouseControllerStatesDumpFile(String objectState) {
		try {
			...
			ObjectInputStream inputStream = new ObjectInputStream(new FileInputStream(objectState));
			GreenhouseControls greenhouseControls = (GreenhouseControls) inputStream.readObject();
			setCurrentController(greenhouseControls);
			...
			eventTextArea.append("**********************************************************\n\r");
			eventTextArea.append("               Issue Repaired Resuming Program            \n\r");
			eventTextArea.append("**********************************************************\n\r");
